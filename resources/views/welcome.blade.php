<!DOCTYPE HTML>
<html lang="en">
    
<!-- Mirrored from promo-theme.com/persona-html/ by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 05 Feb 2018 12:01:19 GMT -->
<head>
        <meta charset=utf-8>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>S.D.Clicks</title>
        <meta name="keywords" content="">
        <meta name="description" content="">
        <link id="favicon" rel="shortcut icon" href="favicon.ico" type="image/x-icon">
        <link href="https://fonts.googleapis.com/css?family=Raleway:100,100i,200,200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i,900,900i" rel="stylesheet">
        {!! Html::style('storage/frontend/css/icons.css') !!}
        {!! Html::style('storage/frontend/css/social-icons.css') !!}
        {!! Html::style('storage/frontend/css/frontend-grid.css') !!}
        {!! Html::style('storage/frontend/css/owl.carousel.css') !!}
        {!! Html::style('storage/frontend/css/style.css') !!}
        {!! Html::style('storage/frontend/css/mobile.css') !!}
        <!--[if IE]>
            <script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/mootools/1.4.0/mootools-yui-compressed.js"></script>
            <script type="text/javascript" src="js/selectivizr-min.js"></script>
            <script src="https://css3-mediaqueries-js.googlecode.com/svn/trunk/css3-mediaqueries.js"></script>
            <script src="http://html5shiv.googlecode.com/svn/trunk/html5.js"></script>
        <![endif]-->
    </head>
    <body>
        <div id="all">
            <!-- Preloader -->
            <div id="page-preloader">
                <div class="spinner centered-container"><img src="{{ url('storage/frontend/images/preloader.jpg') }}" alt=""></div>
            </div>
            <!-- END Preloader -->
            <header class="header fw-main-row">
                <div class="fw-container">
                    <!-- Logo -->
                    <div class="logo"><a href="index-2.html"><img src="{{ url('storage/frontend/images/logo.png') }}" alt=""></a></div>
                    <!-- END Logo -->
                    <div class="fr">
                        <!-- Navigation -->
                        <nav class="navigation">
                            <ul class="menu">
                                <li class="menu-item-has-children current-menu-parent">
                                    <a href="index-2.html">Home</a>
                                    <ul class="sub-menu" style="background-image: {{ url('storage/frontend/images/imgs/bg-sub1.jpg') }} ; padding-left: 200px; left: -185px; min-height: 270px;">
                                        <li class="current-menu-item"><a href="index-2.html">home: photographer</a></li>
                                        <li><a href="photagrapher_2.html">home: photographer 2</a></li>
                                        <li><a href="blog.html">home: Blogger</a></li>
                                        <li><a href="blog_with_sidebar.html">home: Blogger with sidebar</a></li>
                                        <li><a href="blog_gallery.html">home: Blogger + photographer</a></li>
                                    </ul>
                                </li>
                                <li class="menu-item-has-children">
                                    <a href="gallery_grid1.html">gallery</a>
                                    <ul class="sub-menu">
                                        <li class="menu-item-has-children">
                                            <a href="gallery_grid_col2.html">Grid</a>
                                            <ul class="sub-menu">
                                                <li><a href="gallery_grid_col2.html">Grid col 2</a></li>
                                                <li><a href="gallery_grid_col3.html">Grid col 3</a></li>
                                                <li><a href="gallery_grid_col4.html">Grid col 4</a></li>
                                            </ul>
                                        </li>
                                        <li class="menu-item-has-children">
                                            <a href="gallery_masonry_col2.html">Masonry</a>
                                            <ul class="sub-menu">
                                                <li><a href="gallery_masonry_col2.html">Masonry col 2</a></li>
                                                <li><a href="gallery_masonry_col3.html">Masonry col 3</a></li>
                                                <li><a href="gallery_masonry_col4.html">Masonry col 4</a></li>
                                            </ul>
                                        </li>
                                    </ul>
                                </li>
                                <li class="menu-item-has-children">
                                    <a href="blog_masonry.html">blog</a>
                                    <ul class="sub-menu">
                                        <li><a href="blog_masonry.html">Masonry</a></li>
                                        <li><a href="blog_grid.html">Grid</a></li>
                                        <li><a href="blog_grid_with_sidebar.html">Grid with sidebar</a></li>
                                        <li><a href="blog_horizontal.html">horizontal</a></li>
                                        <li><a href="blog_horizontal_with_sidebar.html">horizontal with sidebar</a></li>
                                    </ul>
                                </li>
                                <li class="menu-item-has-children">
                                    <a href="author.html">PAGES</a>
                                    <ul class="sub-menu" style="background-image: url(images/imgs/bg-sub2.jpg); padding-left: 190px; left: -175px; min-height: 280px;">
                                        <li><a href="author.html">author page</a></li>
                                        <li><a href="categories.html">categories</a></li>
                                        <li><a href="coming_soon.html">coming soon</a></li>
                                        <li><a href="404_page.html">404 page</a></li>
                                        <li class="menu-item-has-children">
                                            <a href="#">Menu level</a>
                                            <ul class="sub-menu">
                                                <li class="menu-item-has-children">
                                                    <a href="#">Hyperlink 1</a>
                                                    <ul class="sub-menu">
                                                        <li class="menu-item-has-children">
                                                            <a href="#">Hyperlink 1</a>
                                                            <ul class="sub-menu">
                                                                <li><a href="#">Hyperlink 1</a></li>
                                                                <li><a href="#">Hyperlink 2</a></li>
                                                                <li><a href="#">Hyperlink 3</a></li>
                                                            </ul>
                                                        </li>
                                                        <li class="menu-item-has-children">
                                                            <a href="#">Hyperlink 2</a>
                                                            <ul class="sub-menu">
                                                                <li><a href="#">Hyperlink 1</a></li>
                                                                <li><a href="#">Hyperlink 2</a></li>
                                                                <li><a href="#">Hyperlink 3</a></li>
                                                            </ul>
                                                        </li>
                                                        <li class="menu-item-has-children">
                                                            <a href="#">Hyperlink 3</a>
                                                            <ul class="sub-menu">
                                                                <li><a href="#">Hyperlink 1</a></li>
                                                                <li><a href="#">Hyperlink 2</a></li>
                                                                <li><a href="#">Hyperlink 3</a></li>
                                                            </ul>
                                                        </li>
                                                    </ul>
                                                </li>
                                                <li class="menu-item-has-children">
                                                    <a href="#">Hyperlink 2</a>
                                                    <ul class="sub-menu">
                                                        <li class="menu-item-has-children">
                                                            <a href="#">Hyperlink 1</a>
                                                            <ul class="sub-menu">
                                                                <li><a href="#">Hyperlink 1</a></li>
                                                                <li><a href="#">Hyperlink 2</a></li>
                                                                <li><a href="#">Hyperlink 3</a></li>
                                                            </ul>
                                                        </li>
                                                        <li class="menu-item-has-children">
                                                            <a href="#">Hyperlink 2</a>
                                                            <ul class="sub-menu">
                                                                <li><a href="#">Hyperlink 1</a></li>
                                                                <li><a href="#">Hyperlink 2</a></li>
                                                                <li><a href="#">Hyperlink 3</a></li>
                                                            </ul>
                                                        </li>
                                                        <li class="menu-item-has-children">
                                                            <a href="#">Hyperlink 3</a>
                                                            <ul class="sub-menu">
                                                                <li><a href="#">Hyperlink 1</a></li>
                                                                <li><a href="#">Hyperlink 2</a></li>
                                                                <li><a href="#">Hyperlink 3</a></li>
                                                            </ul>
                                                        </li>
                                                    </ul>
                                                </li>
                                                <li class="menu-item-has-children">
                                                    <a href="#">Hyperlink 3</a>
                                                    <ul class="sub-menu">
                                                        <li class="menu-item-has-children">
                                                            <a href="#">Hyperlink 1</a>
                                                            <ul class="sub-menu">
                                                                <li><a href="#">Hyperlink 1</a></li>
                                                                <li><a href="#">Hyperlink 2</a></li>
                                                                <li><a href="#">Hyperlink 3</a></li>
                                                            </ul>
                                                        </li>
                                                        <li class="menu-item-has-children">
                                                            <a href="#">Hyperlink 2</a>
                                                            <ul class="sub-menu">
                                                                <li><a href="#">Hyperlink 1</a></li>
                                                                <li><a href="#">Hyperlink 2</a></li>
                                                                <li><a href="#">Hyperlink 3</a></li>
                                                            </ul>
                                                        </li>
                                                        <li class="menu-item-has-children">
                                                            <a href="#">Hyperlink 3</a>
                                                            <ul class="sub-menu">
                                                                <li><a href="#">Hyperlink 1</a></li>
                                                                <li><a href="#">Hyperlink 2</a></li>
                                                                <li><a href="#">Hyperlink 3</a></li>
                                                            </ul>
                                                        </li>
                                                    </ul>
                                                </li>
                                                <li><a href="#">Hyperlink 4</a></li>
                                                <li><a href="#">Hyperlink 5</a></li>
                                            </ul>
                                        </li>
                                        <li><a href="typography.html">typography</a></li>
                                    </ul>
                                </li>
                                <li><a href="contact.html">CONTACT</a></li>
                            </ul>
                        </nav>
                        <!-- END Navigation -->
                        <!-- Search button -->
                        <div class="search"><i class="icon-magnifying-glass"></i></div>
                        <!-- END Search button -->
                        <!-- Side panel button -->
                        <div class="side-panel-button"><div></div></div>
                        <!-- END Side panel button -->
                    </div>
                    <!-- Search form -->
                    <form action="#" class="header-search-form">
                        <button type="submit" class="submit"><i class="icon-magnifying-glass"></i></button>
                        <input type="text" class="input" placeholder="Search...">
                        <div class="button-close"><i class="icon-access-denied"></i></div>
                    </form>
                    <!-- END Search form -->
                </div>
            </header>
            <!-- Side panel -->
            <div class="side-panel-area dark-bg">
                <div class="button-close"><i class="icon-access-denied"></i></div>
                <div class="wrap">
                    <!-- Mobile navigation -->
                    <div class="spa-widget mob-navigation">
                        <h5>Navigation</h5>
                        <ul class="menu">
                            <li class="menu-item-has-children current-menu-parent">
                                <a href="index-2.html">Home</a>
                                <ul class="sub-menu">
                                    <li class="current-menu-item"><a href="index-2.html">Home: Photographer</a></li>
                                    <li><a href="photagrapher_2.html">Home: Photographer 2</a></li>
                                    <li><a href="blog.html">Home: Blogger</a></li>
                                    <li><a href="blog_with_sidebar.html">Home: Blogger with sidebar</a></li>
                                    <li><a href="blog_gallery.html">Home: Blogger + photographer</a></li>
                                </ul>
                            </li>
                            <li class="menu-item-has-children">
                                <a href="gallery_grid1.html">Gallery</a>
                                <ul class="sub-menu">
                                    <li class="menu-item-has-children">
                                        <a href="gallery_grid_col2.html">Grid</a>
                                        <ul class="sub-menu">
                                            <li><a href="gallery_grid_col2.html">Grid col 2</a></li>
                                            <li><a href="gallery_grid_col3.html">Grid col 3</a></li>
                                            <li><a href="gallery_grid_col4.html">Grid col 4</a></li>
                                        </ul>
                                    </li>
                                    <li class="menu-item-has-children">
                                        <a href="gallery_masonry_col2.html">Masonry</a>
                                        <ul class="sub-menu">
                                            <li><a href="gallery_masonry_col2.html">Masonry col 2</a></li>
                                            <li><a href="gallery_masonry_col3.html">Masonry col 3</a></li>
                                            <li><a href="gallery_masonry_col4.html">Masonry col 4</a></li>
                                        </ul>
                                    </li>
                                </ul>
                            </li>
                            <li class="menu-item-has-children">
                                <a href="blog_masonry.html">Blog</a>
                                <ul class="sub-menu">
                                    <li><a href="blog_masonry.html">Masonry</a></li>
                                    <li><a href="blog_grid.html">Grid</a></li>
                                    <li><a href="blog_grid_with_sidebar.html">Grid with sidebar</a></li>
                                    <li><a href="blog_horizontal.html">Horizontal</a></li>
                                    <li><a href="blog_horizontal_with_sidebar.html">Horizontal with sidebar</a></li>
                                </ul>
                            </li>
                            <li class="menu-item-has-children">
                                <a href="author.html">PAGES</a>
                                <ul class="sub-menu">
                                    <li><a href="author.html">Author page</a></li>
                                    <li><a href="categories.html">Categories</a></li>
                                    <li><a href="coming_soon.html">Coming soon</a></li>
                                    <li><a href="404_page.html">404 page</a></li>
                                    <li class="menu-item-has-children">
                                        <a href="#">Menu level</a>
                                        <ul class="sub-menu">
                                            <li class="menu-item-has-children">
                                                <a href="#">Hyperlink 1</a>
                                                <ul class="sub-menu">
                                                    <li class="menu-item-has-children">
                                                        <a href="#">Hyperlink 1</a>
                                                        <ul class="sub-menu">
                                                            <li><a href="#">Hyperlink 1</a></li>
                                                            <li><a href="#">Hyperlink 2</a></li>
                                                            <li><a href="#">Hyperlink 3</a></li>
                                                        </ul>
                                                    </li>
                                                    <li class="menu-item-has-children">
                                                        <a href="#">Hyperlink 2</a>
                                                        <ul class="sub-menu">
                                                            <li><a href="#">Hyperlink 1</a></li>
                                                            <li><a href="#">Hyperlink 2</a></li>
                                                            <li><a href="#">Hyperlink 3</a></li>
                                                        </ul>
                                                    </li>
                                                    <li class="menu-item-has-children">
                                                        <a href="#">Hyperlink 3</a>
                                                        <ul class="sub-menu">
                                                            <li><a href="#">Hyperlink 1</a></li>
                                                            <li><a href="#">Hyperlink 2</a></li>
                                                            <li><a href="#">Hyperlink 3</a></li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </li>
                                            <li class="menu-item-has-children">
                                                <a href="#">Hyperlink 2</a>
                                                <ul class="sub-menu">
                                                    <li class="menu-item-has-children">
                                                        <a href="#">Hyperlink 1</a>
                                                        <ul class="sub-menu">
                                                            <li><a href="#">Hyperlink 1</a></li>
                                                            <li><a href="#">Hyperlink 2</a></li>
                                                            <li><a href="#">Hyperlink 3</a></li>
                                                        </ul>
                                                    </li>
                                                    <li class="menu-item-has-children">
                                                        <a href="#">Hyperlink 2</a>
                                                        <ul class="sub-menu">
                                                            <li><a href="#">Hyperlink 1</a></li>
                                                            <li><a href="#">Hyperlink 2</a></li>
                                                            <li><a href="#">Hyperlink 3</a></li>
                                                        </ul>
                                                    </li>
                                                    <li class="menu-item-has-children">
                                                        <a href="#">Hyperlink 3</a>
                                                        <ul class="sub-menu">
                                                            <li><a href="#">Hyperlink 1</a></li>
                                                            <li><a href="#">Hyperlink 2</a></li>
                                                            <li><a href="#">Hyperlink 3</a></li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </li>
                                            <li class="menu-item-has-children">
                                                <a href="#">Hyperlink 3</a>
                                                <ul class="sub-menu">
                                                    <li class="menu-item-has-children">
                                                        <a href="#">Hyperlink 1</a>
                                                        <ul class="sub-menu">
                                                            <li><a href="#">Hyperlink 1</a></li>
                                                            <li><a href="#">Hyperlink 2</a></li>
                                                            <li><a href="#">Hyperlink 3</a></li>
                                                        </ul>
                                                    </li>
                                                    <li class="menu-item-has-children">
                                                        <a href="#">Hyperlink 2</a>
                                                        <ul class="sub-menu">
                                                            <li><a href="#">Hyperlink 1</a></li>
                                                            <li><a href="#">Hyperlink 2</a></li>
                                                            <li><a href="#">Hyperlink 3</a></li>
                                                        </ul>
                                                    </li>
                                                    <li class="menu-item-has-children">
                                                        <a href="#">Hyperlink 3</a>
                                                        <ul class="sub-menu">
                                                            <li><a href="#">Hyperlink 1</a></li>
                                                            <li><a href="#">Hyperlink 2</a></li>
                                                            <li><a href="#">Hyperlink 3</a></li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </li>
                                            <li><a href="#">Hyperlink 4</a></li>
                                            <li><a href="#">Hyperlink 5</a></li>
                                        </ul>
                                    </li>
                                    <li><a href="typography.html">Typography</a></li>
                                </ul>
                            </li>
                            <li><a href="contact.html">Contact</a></li>
                        </ul>
                    </div>
                    <!-- END Mobile navigation -->
                    <!-- Widget item -->
                    <div class="spa-widget">
                        <h5>About me</h5>
                        <div class="content">
                            <img src="images/imgs/about-widget.jpg" alt="Ella">
                            <p>My name is Ella. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula. Mauris risus massa, hendrerit in blandit eu, placerat nec nulla.</p>
                        </div>
                    </div>
                    <!-- END Widget -->
                    <!-- Widget item -->
                    <div class="spa-widget">
                        <h5>Gallery</h5>
                        <div class="gallery-widget fw-row">
                            <div class="fw-col-xs-4"><a href="gallery-post.html"><img src="{{ url('storage/frontend/images/imgs/gallery-widget1.jpg') }}" alt=""></a></div>
                            <div class="fw-col-xs-4"><a href="gallery-post.html"><img src="{{ url('storage/frontend/images/imgs/gallery-widget2.jpg') }}" alt=""></a></div>
                            <div class="fw-col-xs-4"><a href="gallery-post.html"><img src="{{ url('storage/frontend/images/imgs/gallery-widget3.jpg') }}" alt=""></a></div>
                            <div class="fw-col-xs-4"><a href="gallery-post.html"><img src="{{ url('storage/frontend/images/imgs/gallery-widget4.jpg') }}" alt=""></a></div>
                            <div class="fw-col-xs-4"><a href="gallery-post.html"><img src="{{ url('storage/frontend/images/imgs/gallery-widget5.jpg') }}" alt=""></a></div>
                            <div class="fw-col-xs-4"><a href="gallery-post.html"><img src="{{ url('storage/frontend/images/imgs/gallery-widget6.jpg') }}" alt=""></a></div>
                        </div>
                    </div>
                    <!-- END Widget -->
                    <!-- Widget item -->
                    <div class="spa-widget">
                        <h5>contacts</h5>
                        <div class="contact-row">
                            <div class="label">Adress:</div>
                            <div class="value">London, Baker Street 211</div>
                        </div>
                        <div class="contact-row">
                            <div class="label">Phone:</div>
                            <div class="value">+1 (223) 213 2233</div>
                        </div>
                        <div class="contact-row">
                            <div class="label">E-mail:</div>
                            <div class="value">support@promo-theme.com</div>
                        </div>
                    </div>
                    <!-- END Widget -->
                    <!-- Widget item -->
                    <div class="spa-widget">
                        <h5>categories</h5>
                        <ul class="menu">
                            <li><a href="blog_grid.html">Travel</a></li>
                            <li><a href="blog_grid.html">Fashion</a></li>
                            <li><a href="blog_grid.html">Nature</a></li>
                            <li><a href="blog_grid.html">Friends</a></li>
                            <li><a href="blog_grid.html">Life</a></li>
                            <li><a href="blog_grid.html">Work</a></li>
                        </ul>
                    </div>
                    <!-- END Widget -->
                    <!-- Widget item -->
                    <div class="spa-widget">
                        <h5>Popular tags</h5>
                        <div class="tags-items min">
                            <a href="blog_grid.html">nutrition</a>
                            <a href="blog_grid.html">friends</a>
                            <a href="blog_grid.html">thoughts</a>
                            <a href="blog_grid.html">travel</a>
                            <a href="blog_grid.html">life</a>
                            <a href="blog_grid.html">fashion</a>
                            <a href="blog_grid.html">health</a>
                            <a href="blog_grid.html">work</a>
                        </div>
                    </div>
                    <!-- END Widget -->
                    <!-- Copyright -->
                    <div class="copyrighting">(c) Persona, 2016,<br>Designed and depeloped by Promo-theme.com</div>
                    <!-- END Copyright -->
                </div>
            </div>
            <!-- END Side panel -->
            <!-- Banner block -->
            <div class="banner-block full-screen-block fw-main-row">
                <div class="item dark-bg" style="background-image: url(images/imgs/slide2.jpg);">
                    <div class="centered-container fw-container">
                        <div class="h1">I’m a Photographer</div>
                        <p>Your dream about your photo will become true with me</p>
                        <a href="#" class="button-style1"><span>purchase</span></a>
                    </div>
                </div>
            </div>
            <!-- END Banner block -->
            <section class="fw-main-row">
                <div class="fw-container-fluid">
                    <!-- Heading -->
                    <div class="heading-decor">
                        <h5>Choose on of my</h5>
                        <h2>Categories</h2>
                    </div>
                    <!-- END Heading -->
                    <p class="p-text tac" style="max-width:630px;margin: 0 auto 25px;">Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula. Mauris risus massa, hendrerit in blandit eu, placerat nec nulla.</p>
                    <!-- Category items -->
                    <div class="fw-row">
                        <!-- Category item -->
                        <div class="fw-col-xs-12 fw-col-sm-6 fw-col-md-3"><div class="category-item dark-bg" style="background-image: url(images/imgs/blog-category5.jpg);"><img src="images/imgs/blog-category5.jpg" alt="portrait"><a href="blog_grid_with_sidebar.html" class="button-style2 centered-container"><span>portrait</span></a></div></div>
                        <!-- END Category item -->
                        <!-- Category item -->
                        <div class="fw-col-xs-12 fw-col-sm-6 fw-col-md-3"><div class="category-item dark-bg" style="background-image: url(images/imgs/blog-category6.jpg);"><img src="images/imgs/blog-category6.jpg" alt="lifestyle"><a href="blog_grid_with_sidebar.html" class="button-style2 centered-container"><span>lifestyle</span></a></div></div>
                        <!-- END Category item -->
                        <!-- Category item -->
                        <div class="fw-col-xs-12 fw-col-sm-6 fw-col-md-3"><div class="category-item dark-bg" style="background-image: url(images/imgs/blog-category7.jpg);"><img src="images/imgs/blog-category7.jpg" alt="fashion"><a href="blog_grid_with_sidebar.html" class="button-style2 centered-container"><span>fashion</span></a></div></div>
                        <!-- END Category item -->
                        <!-- Category item -->
                        <div class="fw-col-xs-12 fw-col-sm-6 fw-col-md-3"><div class="category-item dark-bg" style="background-image: url(images/imgs/blog-category8.jpg);"><img src="images/imgs/blog-category8.jpg" alt="people"><a href="blog_grid_with_sidebar.html" class="button-style2 centered-container"><span>people</span></a></div></div>
                        <!-- END Category item -->
                    </div>
                    <!-- END Category items -->
                </div>
            </section>
            <!-- Container with side image -->
            <section class="fw-main-row">
                <div class="fw-container-fluid">
                    <div class="fw-row">
                        <div class="col-img fw-col-xs-12 fw-col-sm-4 fw-col-md-6">
                            <div style="background-image: url(images/imgs/side-bg1.jpg);"><img src="images/imgs/side-bg1.jpg" alt="about me"></div>
                        </div>
                        <div class="fw-col-xs-12 fw-col-sm-8 fw-col-md-6">
                            <!-- Heading -->
                            <div class="heading-decor">
                                <h5>few words</h5>
                                <h2>about me</h2>
                            </div>
                            <!-- END Heading -->
                            <p class="p-text tac">My name is Angelina Parker, and I’m a professional photographer. Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula. Mauris risus massa, hendrerit in blandit eu, placerat nec nulla.</p>
                            <h4 class="tac uppercase">My work principles</h4>
                            <div class="fw-row">
                                <!-- Number block -->
                                <div class="fw-col-xs-12 fw-col-lg-6">
                                    <div class="number-block-item">
                                        <div class="num">01</div>
                                        <div class="text">
                                            <div class="h5 uppercase">TAKE A WALK</div>
                                            <p>Nullam sit amet mattis sem, ut hendrerit sapien. Mauris suscipit feugiat ante quis pellentesque.</p>
                                        </div>
                                    </div>
                                </div>
                                <!-- END Number block -->
                                <!-- Number block -->
                                <div class="fw-col-xs-12 fw-col-lg-6">
                                    <div class="number-block-item">
                                        <div class="num">02</div>
                                        <div class="text">
                                            <div class="h5 uppercase">make a decision</div>
                                            <p>Nullam sit amet mattis sem, ut hendrerit sapien. Mauris suscipit feugiat ante quis pellentesque.</p>
                                        </div>
                                    </div>
                                </div>
                                <!-- END Number block -->
                                <!-- Number block -->
                                <div class="fw-col-xs-12 fw-col-lg-6">
                                    <div class="number-block-item">
                                        <div class="num">03</div>
                                        <div class="text">
                                            <div class="h5 uppercase">BUY A TICKET</div>
                                            <p>Nullam sit amet mattis sem, ut hendrerit sapien. Mauris suscipit feugiat ante quis pellentesque.</p>
                                        </div>
                                    </div>
                                </div>
                                <!-- END Number block -->
                                <!-- Number block -->
                                <div class="fw-col-xs-12 fw-col-lg-6">
                                    <div class="number-block-item">
                                        <div class="num">04</div>
                                        <div class="text">
                                            <div class="h5 uppercase">FLY TO NOWHERE</div>
                                            <p>Nullam sit amet mattis sem, ut hendrerit sapien. Mauris suscipit feugiat ante quis pellentesque.</p>
                                        </div>
                                    </div>
                                </div>
                                <!-- END Number block -->
                            </div>
                            <div class="tac" style="margin-top: 25px;"><a href="#" class="button-style3 white"><i class="icon-right-thin-chevron"></i><span>know MORE</span></a></div>
                        </div>
                    </div>
                    <div class="heading-decor">
                        <h5>enjoy my</h5>
                        <h2>portfolio</h2>
                    </div>
                    <div class="portfolio-items masonry fw-row">
                        <!-- Portfolio item -->
                        <div class="portfolio-item fw-col-xs-12 fw-col-sm-6 fw-col-md-3">
                            <div style="background-image: url(images/imgs/gallery-item2.jpg);">
                                <a href="gallery-post.html">
                                    <div>
                                        <div class="h5">Photo name</div>
                                        <span>Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula.</span>
                                    </div>
                                </a>
                            </div>
                        </div>
                        <!-- END Portfolio item -->
                        <!-- Portfolio item -->
                        <div class="portfolio-item fw-col-xs-12 fw-col-sm-6 fw-col-md-6">
                            <div style="background-image: url(images/imgs/gallery-item1.jpg);">
                                <a href="gallery-post.html">
                                    <div>
                                        <div class="h5">Photo name</div>
                                        <span>Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula.</span>
                                    </div>
                                </a>
                            </div>
                        </div>
                        <!-- END Portfolio item -->
                        <!-- Portfolio item -->
                        <div class="portfolio-item fw-col-xs-12 fw-col-sm-6 fw-col-md-3">
                            <div style="background-image: url(images/imgs/gallery-item3.jpg);">
                                <a href="gallery-post.html">
                                    <div>
                                        <div class="h5">Photo name</div>
                                        <span>Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula.</span>
                                    </div>
                                </a>
                            </div>
                        </div>
                        <!-- END Portfolio item -->
                        <!-- Portfolio item -->
                        <div class="portfolio-item fw-col-xs-12 fw-col-sm-6 fw-col-md-3">
                            <div style="background-image: url(images/imgs/gallery-item4.jpg);">
                                <a href="gallery-post.html">
                                    <div>
                                        <div class="h5">Photo name</div>
                                        <span>Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula.</span>
                                    </div>
                                </a>
                            </div>
                        </div>
                        <!-- END Portfolio item -->
                        <!-- Portfolio item -->
                        <div class="portfolio-item fw-col-xs-12 fw-col-sm-6 fw-col-md-3">
                            <div style="background-image: url(images/imgs/gallery-item5.jpg);">
                                <a href="gallery-post.html">
                                    <div>
                                        <div class="h5">Photo name</div>
                                        <span>Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula.</span>
                                    </div>
                                </a>
                            </div>
                        </div>
                        <!-- END Portfolio item -->
                        <!-- Portfolio item -->
                        <div class="portfolio-item fw-col-xs-12 fw-col-sm-6 fw-col-md-3">
                            <div style="background-image: url(images/imgs/gallery-item6.jpg);">
                                <a href="gallery-post.html">
                                    <div>
                                        <div class="h5">Photo name</div>
                                        <span>Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula.</span>
                                    </div>
                                </a>
                            </div>
                        </div>
                        <!-- END Portfolio item -->
                        <!-- Portfolio item -->
                        <div class="portfolio-item fw-col-xs-12 fw-col-sm-6 fw-col-md-3">
                            <div style="background-image: url(images/imgs/gallery-item7.jpg);">
                                <a href="gallery-post.html">
                                    <div>
                                        <div class="h5">Photo name</div>
                                        <span>Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula.</span>
                                    </div>
                                </a>
                            </div>
                        </div>
                        <!-- END Portfolio item -->
                        <!-- Portfolio item -->
                        <div class="portfolio-item row1 fw-col-xs-12 fw-col-sm-6 fw-col-md-6">
                            <div style="background-image: url(images/imgs/gallery-item8.jpg);">
                                <a href="gallery-post.html">
                                    <div>
                                        <div class="h5">Photo name</div>
                                        <span>Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula.</span>
                                    </div>
                                </a>
                            </div>
                        </div>
                        <!-- END Portfolio item -->
                    </div>
                    <div class="tac" style="margin-top: 25px;"><a href="#" class="button-style3 white"><i class="icon-down-chevron"></i><span>LOAD MORE</span></a></div>
                </div>
            </section>
            <!-- END Container with side image -->
            <!-- Testimonials -->
            <section class="fw-main-row">
                <div class="fw-container">
                    <div class="heading-decor">
                        <h5>my clients</h5>
                        <h2>testimotionals</h2>
                    </div>
                    <!-- Slider -->
                    <div class="slider-wrap">
                        <div class="item testimonials-item">
                            <div class="avatar"><img src="images/imgs/avatar.jpg" alt="Emma Levise"></div>
                            <blockquote>Mauris suscipit feugiat ante quis pellentesque. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus non pharetra arcu. Quisque vitae molestie ex. In placerat dolor magna, non pretium elit pellentesque id. Maecenas at neque blandit, commodo augue in. <span class="post">Emma Levise</span></blockquote>
                        </div>
                        <div class="item testimonials-item">
                            <div class="avatar"><img src="images/imgs/avatar.jpg" alt="Emma Levise"></div>
                            <blockquote>Mauris suscipit feugiat ante quis pellentesque. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus non pharetra arcu. Quisque vitae molestie ex. In placerat dolor magna, non pretium elit pellentesque id. Maecenas at neque blandit, commodo augue in. <span class="post">Emma Levise</span></blockquote>
                        </div>
                        <div class="item testimonials-item">
                            <div class="avatar"><img src="images/imgs/avatar.jpg" alt="Emma Levise"></div>
                            <blockquote>Mauris suscipit feugiat ante quis pellentesque. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus non pharetra arcu. Quisque vitae molestie ex. In placerat dolor magna, non pretium elit pellentesque id. Maecenas at neque blandit, commodo augue in. <span class="post">Emma Levise</span></blockquote>
                        </div>
                        <div class="item testimonials-item">
                            <div class="avatar"><img src="images/imgs/avatar.jpg" alt="Emma Levise"></div>
                            <blockquote>Mauris suscipit feugiat ante quis pellentesque. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus non pharetra arcu. Quisque vitae molestie ex. In placerat dolor magna, non pretium elit pellentesque id. Maecenas at neque blandit, commodo augue in. <span class="post">Emma Levise</span></blockquote>
                        </div>
                        <div class="item testimonials-item">
                            <div class="avatar"><img src="images/imgs/avatar.jpg" alt="Emma Levise"></div>
                            <blockquote>Mauris suscipit feugiat ante quis pellentesque. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus non pharetra arcu. Quisque vitae molestie ex. In placerat dolor magna, non pretium elit pellentesque id. Maecenas at neque blandit, commodo augue in. <span class="post">Emma Levise</span></blockquote>
                        </div>
                    </div>
                    <!-- END Slider -->
                </div>
            </section>
            <!-- END Testimonials -->
            <section class="fw-main-row dark-overlay" style="padding-bottom: 30px;">
                <div class="fw-container-fluid">
                    <div class="heading-decor">
                        <h5>follow me on my</h5>
                        <h2>instagram</h2>
                    </div>
                    <p class="p-text tac" style="max-width:630px;margin: 0 auto 25px;">Suspendisse potenti. Pellentesque cursus pulvinar pellentesque. Aenean euismod non lorem at vehicula. Mauris risus massa, hendrerit in blandit eu, placerat nec nulla.</p>
                    <!-- Carousel -->
                    <div class="photo-carousel">
                        <div class="item" style="background-image: url(images/imgs/blog-category1.jpg)"><a href="#"></a><img src="images/imgs/blog-category1.jpg" alt="carousel item"></div>
                        <div class="item" style="background-image: url(images/imgs/blog-category2.jpg)"><a href="#"></a><img src="images/imgs/blog-category2.jpg" alt="carousel item"></div>
                        <div class="item" style="background-image: url(images/imgs/blog-category3.jpg)"><a href="#"></a><img src="images/imgs/blog-category3.jpg" alt="carousel item"></div>
                        <div class="item" style="background-image: url(images/imgs/blog-category4.jpg)"><a href="#"></a><img src="images/imgs/blog-category4.jpg" alt="carousel item"></div>
                        <div class="item" style="background-image: url(images/imgs/blog-category5.jpg)"><a href="#"></a><img src="images/imgs/blog-category5.jpg" alt="carousel item"></div>
                        <div class="item" style="background-image: url(images/imgs/blog-category6.jpg)"><a href="#"></a><img src="images/imgs/blog-category6.jpg" alt="carousel item"></div>
                        <div class="item" style="background-image: url(images/imgs/blog-category7.jpg)"><a href="#"></a><img src="images/imgs/blog-category7.jpg" alt="carousel item"></div>
                        <div class="item" style="background-image: url(images/imgs/blog-category8.jpg)"><a href="#"></a><img src="images/imgs/blog-category8.jpg" alt="carousel item"></div>
                    </div>
                    <!-- END Carousel -->
                    <div class="tac"><a href="#" class="button-style3 big white-dark"><i class="social-icon-instagram-social-network-logo-of-photo-camera"></i><span>follow me</span></a></div>
                </div>
            </section>
            <section class="fw-main-row">
                <div class="fw-container-fluid">
                    <div class="heading-decor">
                        <h5>choose one of my</h5>
                        <h2>HOT prices</h2>
                    </div>
                    <div class="fw-row">
                        <div class="fw-col-xs-12 fw-col-sm-6 fw-col-lg-3">
                            <!-- Pricing item -->
                            <div class="pricing-item">
                                <div class="top" style="background-image: url(images/imgs/price1.jpg)">
                                    <h5 class="uppercase"><span>outdoor session</span></h5>
                                    <div class="price">49 <span>$</span></div>
                                    <h6>Beautifull</h6>
                                </div>
                                <div class="wrap">
                                    <div class="row">3 hours on location</div>
                                    <div class="row">1 outfit change</div>
                                    <div class="row">50 images</div>
                                    <div class="row">15 edited images</div>
                                    <div class="row">Lo-res images for Facebook</div>
                                    <div class="row">Hi-res images on CD</div>
                                </div>
                                <div class="bottom"><a href="#" class="button-style3 big"><span>purchase it</span></a></div>
                            </div>
                            <!-- END Pricing item -->
                        </div>
                        <div class="fw-col-xs-12 fw-col-sm-6 fw-col-lg-3">
                            <!-- Pricing item -->
                            <div class="pricing-item">
                                <div class="top" style="background-image: url(images/imgs/price2.jpg)">
                                    <h5 class="uppercase"><span>portrait session</span></h5>
                                    <div class="price">99 <span>$</span></div>
                                    <h6>Unforgetable</h6>
                                </div>
                                <div class="wrap">
                                    <div class="row">3 hours on location</div>
                                    <div class="row">1 outfit change</div>
                                    <div class="row">50 images</div>
                                    <div class="row">15 edited images</div>
                                    <div class="row">Lo-res images for Facebook</div>
                                    <div class="row">Hi-res images on CD</div>
                                </div>
                                <div class="bottom"><a href="#" class="button-style3 big"><span>purchase it</span></a></div>
                            </div>
                            <!-- END Pricing item -->
                        </div>
                        <div class="fw-col-xs-12 fw-col-sm-6 fw-col-lg-3">
                            <!-- Pricing item -->
                            <div class="pricing-item">
                                <div class="top" style="background-image: url(images/imgs/price3.jpg)">
                                    <h5 class="uppercase"><span>studio session</span></h5>
                                    <div class="price">129 <span>$</span></div>
                                    <h6>Fashionable</h6>
                                </div>
                                <div class="wrap">
                                    <div class="row">3 hours on location</div>
                                    <div class="row">1 outfit change</div>
                                    <div class="row">50 images</div>
                                    <div class="row">15 edited images</div>
                                    <div class="row">Lo-res images for Facebook</div>
                                    <div class="row">Hi-res images on CD</div>
                                </div>
                                <div class="bottom"><a href="#" class="button-style3 big"><span>purchase it</span></a></div>
                            </div>
                            <!-- END Pricing item -->
                        </div>
                        <div class="fw-col-xs-12 fw-col-sm-6 fw-col-lg-3">
                            <!-- Pricing item -->
                            <div class="pricing-item">
                                <div class="top" style="background-image: url(images/imgs/price4.jpg)">
                                    <h5 class="uppercase"><span>ultimate session</span></h5>
                                    <div class="price">169 <span>$</span></div>
                                    <h6>Unstopable</h6>
                                </div>
                                <div class="wrap">
                                    <div class="row">3 hours on location</div>
                                    <div class="row">1 outfit change</div>
                                    <div class="row">50 images</div>
                                    <div class="row">15 edited images</div>
                                    <div class="row">Lo-res images for Facebook</div>
                                    <div class="row">Hi-res images on CD</div>
                                </div>
                                <div class="bottom"><a href="#" class="button-style3 big"><span>purchase it</span></a></div>
                            </div>
                            <!-- END Pricing item -->
                        </div>
                    </div>
                </div>
            </section>
            <!-- END Container with side image -->
            <footer class="footer dark-overlay fw-main-row">
                <div class="fw-container">
                    <h4 class="tac">Tags</h4>
                    <!-- Tag area -->
                    <div class="tags-items">
                        <a href="blog_grid.html">nutrition</a>
                        <a href="blog_grid.html">friends</a>
                        <a href="blog_grid.html">thoughts</a>
                        <a href="blog_grid.html">travel</a>
                        <a href="blog_grid.html">life</a>
                        <a href="blog_grid.html">fashion</a>
                        <a href="blog_grid.html">health</a>
                        <a href="blog_grid.html">work</a>
                    </div>
                    <!-- END Tag area -->
                </div>
                <div class="wrap">
                    <div class="fw-container">
                        <!-- Logo -->
                        <div class="logo"><a href="index-2.html"><img src="images/logo-light.png" alt=""></a></div>
                        <!-- END Logo -->
                        <!-- Social links -->
                        <div class="social-links">
                            <a href="#"><i class="social-icon-facebook-logo"></i></a>
                            <a href="#"><i class="social-icon-instagram-social-network-logo-of-photo-camera"></i></a>
                            <a href="#"><i class="social-icon-twitter-social-logotype"></i></a>
                        </div>
                        <!-- END Social links -->
                        <div class="center">
                            <!-- Footer menu -->
                            <ul class="menu">
                                <li><a href="#">HOME</a></li>
                                <li><a href="#">CATEGORIES</a></li>
                                <li><a href="#">AUTHOR</a></li>
                                <li><a href="#">PAGES</a></li>
                                <li><a href="#">CONTACT</a></li>
                            </ul>
                            <!-- END Footer menu -->
                            <!-- Copyright -->
                            <div class="copyright">(c) Persona, 2016, Designed and depeloped by Promo-theme.com</div>
                            <!-- END Copyright -->
                        </div>
                    </div>
                </div>
            </footer>
        </div>
        
        {!! Html::script('storage/frontend/js/jquery-2.1.3.min.js') !!}
        {!! Html::script('storage/frontend/js/isotope.pkgd.min.js') !!}
        {!! Html::script('storage/frontend/js/owl.carousel.min.js') !!}
        {!! Html::script('storage/frontend/js/script.js') !!}
    </body>

<!-- Mirrored from promo-theme.com/persona-html/ by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 05 Feb 2018 12:03:07 GMT -->
</html>